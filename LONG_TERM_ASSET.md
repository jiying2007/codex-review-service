...
Codex Review Service is maintained as a security- and reliability-sensitive platform component. Service **v7.5.1** is the current production-operations baseline; shared review/safety/Judgment Lifecycle/Runtime/Provider/Model Routing semantics remain exact-pinned Safe Core Family v4.

## Product identity

`product-contract.json` is the machine-checked product fact source: Service 7.5.1, Database Schema 8, Config Schema 8, Policy Schema 4, Review Receipt 5, Safe Contract 2, Runtime Contract 3, Provider Contract 3, Model Routing Contract 1, Profile Pack 1, Test Impact 1, Analyzer Adapter 1, Judgment Lifecycle 1, exact Safe Core commit `786e3a3fc896e0e623af6fe63dbf814ddd09bad8`, Node 22.22.2+/24.19.0+ LTS support and GitLab >=14.6.1 compatibility.
...
18. Config Schema identity is explicit. Current runtime accepts **Config Schema 8** only; unsupported or missing schema versions fail closed and hidden compatibility precedence is forbidden.
...
40. Current documentation must describe Service v7.5.1, Database Schema 8, Config Schema 8, Review Receipt 5, Runtime/Provider Contract v3, Model Routing Contract v1 and exact Safe Core 4.16.0 accurately; historical labels belong only in changelog/migration history.
...
Keep `main` deployable. README/README.zh-CN/OPERATIONS/SECURITY/ARCHITECTURE/LONG_TERM_ASSET and `product-contract.json` must remain mutually consistent, and CI must fail when product facts drift.
