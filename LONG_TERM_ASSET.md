...
Codex Review Service is maintained as a security- and reliability-sensitive platform component. Service **v6.2.1** is the current production-operations baseline; shared review/safety semantics remain exact-pinned Safe Core Family v4.

## Product identity

`product-contract.json` is the machine-checked product fact source: Service 6.2.1, Database Schema 6, Config Schema 3, Policy Schema 3, Review Receipt 4, Safe Contract 2, Profile Pack 1, Test Impact 1, Analyzer Adapter 1, exact Safe Core commit `e75d27d5f157cacc5e8f6b711355dd5cf4ddfe34`, Node 22.22.2+/24.19.0+ LTS support and GitLab >=14.6.1 compatibility.
...
31. Service 6.2.1 makes Config Schema 2 -> 3 a documented configuration hard cut. Rollback to a Config Schema 2 release requires restoring its matching config; runtime translation is forbidden.
...
40. Current documentation must describe Service v6.2.1, Database Schema 6, Config Schema 3 and Safe Core Family v4 accurately; historical labels belong only in changelog/migration history.
