...
8. External commit statuses are written to the correct source project/ref and bound to a concrete pipeline when GitLab exposes a compatible pipeline ID; fallback behavior remains explicit and observable.
9. Human-resolved discussions are never silently reopened. A recurring issue gets a new current-snapshot discussion.
10. Token usage, queue latency, review latency, publication failures, project-scope health and provider health are observable without logging source code, prompts, raw model output or secrets.
...
13. Standard Deployment stays simple: one Controller process and inline Codex are a fully supported production path. Hardened Deployment may split Codex into the isolated Unix-socket Runner without changing Review/Gate/Outbox semantics.
...
15. Project scope from explicit Projects/Groups is accepted only after complete discovery. A failed/incomplete refresh must preserve the last complete scope and make readiness unhealthy rather than silently dropping repositories.
16. Legacy wildcard scope is webhook-only and must never be represented as exhaustively reconciled.
17. CI dependencies are pinned by immutable commit SHA, and changes to durability, trust boundaries, schema, outbox, project-scope or merge-gate semantics require contract tests before merge.

## Evolution rule

Prefer clean provider/project-scope/storage/Runner interfaces and additive migrations over compatibility layers. Backward-compatible configuration inputs must have explicit precedence and must not create ambiguous behavior. Keep `main` deployable, keep README/README.zh-CN/OPERATIONS/SECURITY/ARCHITECTURE mutually consistent, and do not declare a release mature while any documented invariant is only aspirational rather than executable/tested.
