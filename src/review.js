...
const { runDeterministicAnalyzers } = require('./analyzers');
const { classifyPath, buildReviewEvidenceChunks } = require('./codex-safe-core/context-builder');
const { REVIEW_RECEIPT_SCHEMA_VERSION, fingerprint, validateReviewReceipt } = require('./codex-safe-core/safe-contract');
...
function parseChangedLines(diff) {
  const changed = { new: [], old: [], anchors: { new: {}, old: {} } };
  let oldLine = 0, newLine = 0, active = false, hunk = '';
  for (const raw of String(diff || '').split(/\r?\n/)) {
    ...
    if (!active) continue;
    if (raw.startsWith('+') && !raw.startsWith('+++')) { changed.new.push(newLine); changed.anchors.new[newLine] = normalizeAnchorText(`${hunk}|${raw.slice(1)}`); newLine += 1; }
    else if (raw.startsWith('-') && !raw.startsWith('---')) { changed.old.push(oldLine); changed.anchors.old[oldLine] = normalizeAnchorText(`${hunk}|${raw.slice(1)}`); oldLine += 1; }
    else if (!raw.startsWith('\\')) { oldLine += 1; newLine += 1; }
  }
  return changed;
}
...
function buildSnapshot(mr, diffResult, policy) {
  ...
  for (const entry of diffResult.items || []) {
    const filePath = String(entry.new_path || entry.old_path || '').replace(/\\/g,'/');
    const oldPath = String(entry.old_path || filePath).replace(/\\/g,'/');
    const newPath = String(entry.new_path || filePath).replace(/\\/g,'/');
    const coreKind = classifyPath(filePath), providerGenerated = entry.generated_file === true;
    let state = 'reviewed_text', reason = '';
    ...
    else if ((providerGenerated || coreKind === 'generated') && policy.skipGeneratedFiles) { state='policy_excluded'; reason='generated_file'; }
    else if (coreKind === 'binary') { state='unreviewable'; reason='binary_file'; if (policy.blockUnreviewableFiles) coverageGaps.push(`${filePath}:binary_file`); else advisories.push(`${filePath}:binary_file`); }
    else if (typeof entry.diff !== 'string' || entry.diff.length === 0) { if (entry.renamed_file === true) { state='metadata_only'; reason='rename_only'; } else { state='coverage_gap'; reason='unavailable_unknown'; } }
    if (state !== 'reviewed_text') { if (state === 'coverage_gap') coverageGaps.push(`${filePath || '<unknown>'}:${reason}`); files.push({...entry,path:filePath,old_path:oldPath,new_path:newPath,skipped:true,coverageState:state,skippedReason:reason,coreKind}); continue; }
    ...
    if (!changedLines.new.length && !changedLines.old.length) {
      if (metadataOnlyDiff(entry.diff) || entry.renamed_file === true) { files.push({...entry,path:filePath,old_path:oldPath,new_path:newPath,skipped:true,coverageState:'metadata_only',skippedReason:'metadata_only',coreKind}); continue; }
      coverageGaps.push(`${filePath}:no_changed_lines`); files.push({...entry,path:filePath,old_path:oldPath,new_path:newPath,skipped:true,coverageState:'coverage_gap',skippedReason:'no_changed_lines',coreKind}); continue;
    }
...
function buildPrompt(snapshot, chunk, policy, context={blocks:[]}) {
  ...
  blocks.push('','--- GITLAB MERGE REQUEST DIFF START ---',chunk.diffText,'--- GITLAB MERGE REQUEST DIFF END ---',''); return blocks.filter(v=>v!=='').join('\n');
}
function normalizeFinding(raw,chunk,policy){if(!raw||typeof raw!=='object'||Array.isArray(raw))return{kind:'rejected',reason:'not_object'};const severity=String(raw.severity||''),category=String(raw.category||''),side=String(raw.side||'');if(!SEVERITIES.includes(severity)||!CATEGORIES.has(category)||!SIDES.has(side))return{kind:'rejected',reason:'enum'};const filePath=String(raw.file||'').replace(/\\/g,'/').replace(/^\.\//,'');const file=chunk.files.find(i=>i.path===filePath);if(!file)return{kind:'rejected',reason:'path'};const confidence=Number(raw.confidence);if(!Number.isFinite(confidence)||confidence<0||confidence>1)return{kind:'rejected',reason:'confidence'};if(confidence<policy.minConfidence)return{kind:'filtered',reason:'low_confidence'};const line=Math.round(Number(raw.line));if(!Number.isInteger(line)||line<1)return{kind:'rejected',reason:'line'};if(!file.changedLines[side].includes(line))return{kind:'rejected',reason:'line_not_changed'};let endLine=Math.round(Number(raw.endLine));if(!Number.isInteger(endLine)||endLine<line)endLine=line;const title=String(raw.title||'').trim().replace(/\s+/g,' '),description=String(raw.description||'').trim(),suggestion=String(raw.suggestion||'').trim();if(!title||title.length>160||!description||description.length>1200||suggestion.length>1200)return{kind:'rejected',reason:'text_bounds'};const anchorText=file.changedLines.anchors?.[side]?.[line]||'',anchorHash=sha256(`${filePath}\n${side}\n${normalizeAnchorText(anchorText)}`),findingFingerprint=sha256(`${category}\n${anchorHash}`);return{kind:'accepted',finding:{severity,category,file:filePath,side,line,endLine,title,description,suggestion,confidence,anchorHash,fingerprint:findingFingerprint}};}
...
function consolidateReviews(snapshot,results,policy){const deduped=new Map(),summaries=[];let rejectedFindingCount=0,filteredFindingCount=0,modelFindingCount=0;const deterministic=runDeterministicAnalyzers(snapshot,policy);for(const finding of deterministic.findings)deduped.set(finding.fingerprint,finding);for(const result of results){if(result.summary)summaries.push(result.summary);rejectedFindingCount+=result.rejected;filteredFindingCount+=result.filtered;modelFindingCount+=result.modelFindingCount;for(const finding of result.findings){const previous=deduped.get(finding.fingerprint);if(!previous||finding.confidence>previous.confidence)deduped.set(finding.fingerprint,finding);}}const uncapped=[...deduped.values()].sort((a,b)=>SEVERITY_ORDER[b.severity]-SEVERITY_ORDER[a.severity]||b.confidence-a.confidence||a.file.localeCompare(b.file));const blocking=uncapped.some(f=>passesThreshold(f.severity,policy.blockingSeverity)),truncatedFindingCount=Math.max(0,uncapped.length-policy.maxFindings),allFindings=uncapped.slice(0,policy.maxFindings),findings=allFindings.filter(f=>passesThreshold(f.severity,policy.severityThreshold)),coverageComplete=snapshot.coverageComplete&&rejectedFindingCount===0;return{summary:summaries.join('\n\n').slice(0,1200),findings,allFindings,verdict:!coverageComplete?'incomplete':blocking?'block':findings.length?'needs_attention':snapshot.advisories.length?'needs_attention':'pass',coverageComplete,rejectedFindingCount,filteredFindingCount,truncatedFindingCount,modelFindingCount,deterministicFindingCount:deterministic.findings.length,deterministicViolationCount:deterministic.violations.length,chunkCount:results.length};}
function emptyReview(snapshot){return{summary:'',findings:[],allFindings:[],verdict:snapshot.coverageComplete?(snapshot.advisories.length?'needs_attention':'pass'):'incomplete',coverageComplete:snapshot.coverageComplete,rejectedFindingCount:0,filteredFindingCount:0,truncatedFindingCount:0,modelFindingCount:0,deterministicFindingCount:0,deterministicViolationCount:0,chunkCount:0};}
