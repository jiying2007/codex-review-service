...
async function main(){
  ...
  const service=new ReviewService({config,store,gitlab,logger,telemetry}),flowTracker=new FlowTracker({config,store,planNotificationActions,logger}),publisher=new Publisher({config,store,gitlab,logger}),notifier=new Notifier({config,store,logger});service.startWorkers();publisher.start();notifier.start();
