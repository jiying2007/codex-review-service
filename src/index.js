...
const { installNotificationStore }=require('./notification-store');
...
async function main(){
  ...
  const store=installNotificationStore(new Store(config.dbPath),config),scheduler=installFairScheduling(store),recoveredJobs=store.recoverInterruptedJobs(),recoveredPublications=store.recoverPublications(),recoveredNotifications=store.recoverNotifications();
  ...
  const gitlab=new GitLabClient(config),scopeManager=new ProjectScopeManager(gitlab,config);await scopeManager.refresh();config=applyProjectScope(config,scopeManager);config=await prepareNotificationRoutes(gitlab,config);store.setNotificationConfig(config);logger.info({event:'project_scope_ready',mode:scopeManager.mode,...scopeManager.stats,notificationRoutes:config.notificationRoutesResolved.length});
