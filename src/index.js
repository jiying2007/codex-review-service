...
async function main(){
  ...
  const gitlab=new GitLabClient(config),scopeManager=new ProjectScopeManager(gitlab,config);await scopeManager.refresh();config=applyProjectScope(config,scopeManager);config=await prepareNotificationRoutes(gitlab,config);logger.info({event:'project_scope_ready',mode:scopeManager.mode,...scopeManager.stats,notificationRoutes:config.notificationRoutesResolved.length});
