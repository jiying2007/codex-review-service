...
test('isolated runner reads canonical Schema 8 and remains model-neutral',()=>withConfig({gitlab:{baseUrl:'https://gitlab.test',projects:[1]},review:{timeoutSeconds:77},codex:{path:'codex-x',home:'',selectionStrategy:'fixed',fixedModel:'controller-owned-model',versionPolicy:'warn'},runner:{mode:'isolated',socket:'/run/test-runner.sock'}},()=>{const config=runnerConfig();assert.equal(config.socket,'/run/test-runner.sock');assert.equal(config.codexPath,'codex-x');assert.equal(config.codexHome,'');assert.equal(config.codexModel,'');assert.equal(config.reviewTimeoutSeconds,77);}));
