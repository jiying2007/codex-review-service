...
test('operations sync replaces the whole prior identity sentence without leaving semver fragments',()=>{
  ...
  assert.match(output,/Service 7\.5\.2 uses Config Schema 8 and Database Schema 8 while consuming Runtime\/Provider Contract v3 and Model Routing Contract v1/);
...
test('security sync upgrades stale release/schema routing identity and is idempotent',()=>{
  ...
  const stale=current
    .replace('Codex Review Service **7.5.2**','Codex Review Service **7.4.3**')
...
test('long-term asset sync upgrades stale product identity and is idempotent',()=>{
  ...
  const stale=current
    .replace('Service **v7.5.2** is the current production-operations baseline','Service **v7.3.0** is the current production-operations baseline')
