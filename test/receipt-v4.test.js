...
function withStore(fn) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-review-receipt-v4-'));
  const store = new Store(path.join(dir, 'test.sqlite'));
  try { return fn(store); }
  finally { store.close(); fs.rmSync(dir, { recursive: true, force: true }); }
}

test('schema 7 contains canonical Review Receipt v4 projection columns', () => withStore(store => {
...
test('GitLab MR Review Receipt v4 is validated and stored atomically with run/outbox', () => withStore(store => {
  const startSha = '1'.repeat(40);
  ...
  assert.equal(projection.receipt.schemaVersion, 4);
  ...
  assert.equal(validateReviewReceipt({ ...projection.receipt, schemaVersion: 3 }), null, 'Receipt v3 must remain invalid');

  const jobId = store.enqueue({ projectId: 7, mrIid: 11, startSha, headSha, trigger: 'open', dedupeKey: `snapshot:${startSha}:${headSha}`, maxQueueDepth: 10 });
  const runId = store.saveRunWithOutbox(jobId, review, 5, policy, [
    { projectId: 7, mrIid: 11, type: 'summary', dedupeKey: 'v4:summary', payload: { headSha } }
  ], projection);
...
test('incomplete Service review produces blocked Receipt v4 provenance', () => {
