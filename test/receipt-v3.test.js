...
function withStore(fn) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-review-receipt-v3-'));
...
test('GitLab MR Review Receipt v3 is validated and stored atomically with run/outbox', () => withStore(store => {
  ...
  assert.equal(projection.receipt.schemaVersion, 3);
  ...
  const runId = store.saveRunWithOutbox(jobId, review, 5, policy, [
    { projectId: 7, mrIid: 11, type: 'summary', dedupeKey: 'v3:summary', payload: { headSha } }
  ], projection);
...
test('incomplete Service review produces blocked Receipt v3 provenance', () => {
