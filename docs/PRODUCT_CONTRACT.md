# Product Contract

This file is generated from `product-contract.json` and the exact Safe Core contract. Do not hand-edit version facts.

<!-- BEGIN GENERATED PRODUCT CONTRACT -->

Codex Review Service **7.4.2** owns production operations and GitLab compatibility profiles while consuming the exact-pinned Safe Core quality/review platform. `product-contract.json` is the machine-checked product identity:

```text
Service 7.4.2
DB Schema 8
Config Schema 7
Policy Schema 4
Review Receipt 5
Safe Contract 2
Safe Core 25467922eeebffa93b7c820f2ffa7590c1625381
Quality Platform 3
Review Profile 1
Profile Pack 1
Impact Evidence 2
Test Impact 1
Analyzer Finding 1
Analyzer Adapter 1
Native Node: 22 LTS >=22.22.2 OR 24 LTS >=24.19.0
Canonical Docker Node: 24.19.0
GitLab compatibility floor: 14.6.1
GitLab recommendation: vendor-supported release
```

<!-- END GENERATED PRODUCT CONTRACT -->
