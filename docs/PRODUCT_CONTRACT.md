# Product Contract

This file is generated from `product-contract.json` and the exact Safe Core contract. Do not hand-edit version facts.

<!-- BEGIN GENERATED PRODUCT CONTRACT -->

Codex Review Service **6.5.2** owns production operations and GitLab compatibility profiles while consuming the exact-pinned Safe Core quality/review platform. `product-contract.json` is the machine-checked product identity:

```text
Service 6.5.2
DB Schema 7
Config Schema 4
Policy Schema 4
Review Receipt 4
Safe Contract 2
Safe Core cd9788f1280a217fbe6d0beb59682a85a8b82c4d
Quality Platform 3
Review Profile 1
Profile Pack 1
Impact Evidence 2
Test Impact 1
Analyzer Finding 1
Analyzer Adapter 1
Native Node: 22 LTS >=22.22.2 OR 24 LTS >=24.19.0
Canonical Docker Node: 24.19.0
GitLab compatibility floor: 14.6.1
GitLab recommendation: vendor-supported release
```

<!-- END GENERATED PRODUCT CONTRACT -->
