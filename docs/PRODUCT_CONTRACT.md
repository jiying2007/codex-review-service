# Product Contract

This file is generated from `product-contract.json` and the exact Safe Core contract. Do not hand-edit version facts.

<!-- BEGIN GENERATED PRODUCT CONTRACT -->

Codex Review Service **6.2.1** owns production operations and GitLab compatibility profiles while consuming the exact-pinned Safe Core quality/review platform. `product-contract.json` is the machine-checked product identity:

```text
Service 6.2.1
DB Schema 6
Config Schema 3
Policy Schema 3
Review Receipt 4
Safe Contract 2
Safe Core b71e11ef3eafba5aa4359376c79a792a05ee85eb
Quality Platform 2
Review Profile 1
Profile Pack 1
Impact Evidence 2
Test Impact 1
Analyzer Finding 1
Analyzer Adapter 1
Native Node: 22 LTS >=22.22.2 OR 24 LTS >=24.19.0
Canonical Docker Node: 24.19.0
GitLab compatibility floor: 14.6.1
GitLab recommendation: vendor-supported release
```

<!-- END GENERATED PRODUCT CONTRACT -->
