...
Read `product-contract.json` before deployment. **Codex Review Service 6.3.0** supports native/systemd Node.js **22 LTS >=22.22.2** or **24 LTS >=24.19.0**, GitLab Self-Managed **>=14.6.1**, Database Schema 6 and **Config Schema 3**. The official Docker image uses canonical Node 24.19.0.

Safe Core is exact-pinned to `10393a0035ce5168b3d0e88822af0d74fe85ec6c`. Do not replace the gitlink or copy another Core runtime into a release package.
...
Service 6.3.0 hard-cuts the quality configuration to Config Schema 3. Config Schema 2 is not translated at runtime. Before rollout, rewrite the configuration and remove the retired `review.sarifFiles` field.
