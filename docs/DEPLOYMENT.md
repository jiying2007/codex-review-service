...
Read `product-contract.json` before deployment. **Codex Review Service 6.2.2** supports native/systemd Node.js **22 LTS >=22.22.2** or **24 LTS >=24.19.0**, GitLab Self-Managed **>=14.6.1**, Database Schema 6 and **Config Schema 3**. The official Docker image uses canonical Node 24.19.0.
...
Service 6.2.2 hard-cuts the quality configuration to Config Schema 3. Config Schema 2 is not translated at runtime. Before rollout, rewrite the configuration and remove the retired `review.sarifFiles` field.
