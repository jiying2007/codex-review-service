...
部署前先读取 `product-contract.json`。**Codex Review Service 7.4.2** 支持 Native/systemd Node.js **22 LTS >=22.22.2** 或 **24 LTS >=24.19.0**，GitLab Self-Managed **>=14.6.1**，Database Schema 8、**Config Schema 7**。官方 Docker 镜像使用 canonical Node 24.19.0。

Safe Core 精确固定到 `25467922eeebffa93b7c820f2ffa7590c1625381`。禁止替换 gitlink 或把另一份 Core Runtime 复制进 Release。
