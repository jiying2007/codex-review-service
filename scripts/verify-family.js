...
const release=fs.readFileSync(path.join(root,'.github/workflows/release.yml'),'utf8');assert.match(release,/branches:\s*\[main\]/);assert.match(release,/github\.event\.comment\.body == '\/release-retry'/);assert.doesNotMatch(release,/--clobber/);assert.match(release,/Release .* already exists; immutable assets will not be overwritten/);assert.match(release,/docker buildx build[\s\S]*--platform linux\/amd64,linux\/arm64[\s\S]*--push/);assert.match(release,/IMAGE_DIGEST\.txt/);assert.match(release,/compose\.release\.yaml/);assert.match(release,/sha256sum "\$tgz" SBOM\.spdx\.json IMAGE_DIGEST\.txt compose\.release\.yaml > SHA256SUMS/);assert.match(release,/attest-build-provenance@4d101475d8b20a2381f78447822ac1eab6504dd8/);assert.match(release,/trivy-action@ed142fd0673e97e23eac54620cfb913e5ce36c25/);

const ci=fs.readFileSync(path.join(root,'.github/workflows/ci.yml'),'utf8'),security=fs.readFileSync(path.join(root,'.github/workflows/security.yml'),'utf8'),gitlabMatrix=fs.readFileSync(path.join(root,'.github/workflows/gitlab-system.yml'),'utf8');assert.match(ci,/docker-smoke/);assert.match(ci,/recovery-gate/);assert.match(security,/dependency-review-action@a1d282b36b6f3519aa1f3fc636f609c47dddb294/);assert.match(security,/codeql-action\/init@99df26d4f13ea111d4ec1a7dddef6063f76b97e9/);assert.match(gitlabMatrix,/19\.1\.6-ce\.0/);assert.match(gitlabMatrix,/scripts\/gitlab-system-smoke\.js/);
