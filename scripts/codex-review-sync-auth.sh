...
CONTAINER="${CODEX_REVIEW_CONTAINER:-codex-review-v510-release-codex-review-service-1}"
SOURCE_AUTH="${CODEX_REVIEW_HOST_AUTH_FILE:-$HOME/.codex/auth.json}"
IMAGE="${CODEX_REVIEW_IMAGE:-codex-review-service:local-5.1.0-02113d0}"
...
if [ "$MODE" = dry-run ]; then echo "Would sync host Codex auth into Docker volume: $VOLUME"; exit 0; fi
