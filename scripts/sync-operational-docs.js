...
function syncLongTermAsset(text){
  let out=text;
  out=replaceRequired(out,/Service \*\*v\d+\.\d+\.\d+\*\* is the current production-operations baseline/,`Service **v${contract.serviceVersion}** is the current production-operations baseline`,'long-term service version');
  out=replaceRequired(out,/`product-contract\.json` is the machine-checked product fact source:[^\n]+/,`\`product-contract.json\` is the machine-checked product fact source: Service ${contract.serviceVersion}, Database Schema ${contract.databaseSchemaVersion}, Config Schema ${contract.configSchemaVersion}, Policy Schema ${contract.policySchemaVersion}, Review Receipt ${contract.reviewReceiptVersion}, Safe Contract ${contract.safeContractVersion}, Runtime Contract ${contract.codexRuntimeVersion}, Provider Contract ${contract.providerContractVersion}, Model Routing Contract ${contract.modelRoutingContractVersion}, Profile Pack ${contract.profilePackVersion}, Test Impact ${contract.testImpactVersion}, Analyzer Adapter ${contract.analyzerAdapterVersion}, Judgment Lifecycle ${contract.judgmentLifecycleVersion}, exact Safe Core commit \`${contract.safeCoreCommit}\`, Node ${contract.minimumNodeVersion}+/${contract.canonicalNodeVersion}+ LTS support and GitLab >=${contract.minimumGitLabVersion} compatibility.`,'long-term product identity');
  out=replaceRequired(out,/18\. Config Schema identity is explicit\. Current runtime accepts \*\*Config Schema \d+\*\* only; unsupported or missing schema versions fail closed and hidden compatibility precedence is forbidden\./,`18. Config Schema identity is explicit. Current runtime accepts **Config Schema ${contract.configSchemaVersion}** only; unsupported or missing schema versions fail closed and hidden compatibility precedence is forbidden.`,'long-term config invariant');
  out=replaceRequired(out,/31\. Service .*?runtime translation or in-place Schema downgrade is forbidden\./,`31. Service ${contract.serviceVersion} makes Config Schema 7 -> ${contract.configSchemaVersion} the Model Routing Contract v${contract.modelRoutingContractVersion} hard cut. Service 7.3.0 made Config Schema 6 -> 7 the Provider Contract hard cut, Service 7.2.0 made Config Schema 5 -> 6 the responsibility-notification hard cut, and Service 7.0.0 made Config Schema 4 -> 5 the Judgment Lifecycle hard cut. Rollback across database/config boundaries requires the matching verified backup and target-release configuration; runtime translation or in-place Schema downgrade is forbidden.`,'long-term migration invariant');
  out=replaceRequired(out,/40\. Current documentation must describe Service .*?historical labels belong only in changelog\/migration history\./,`40. Current documentation must describe Service v${contract.serviceVersion}, Database Schema ${contract.databaseSchemaVersion}, Config Schema ${contract.configSchemaVersion}, Review Receipt ${contract.reviewReceiptVersion}, Runtime/Provider Contract v${contract.providerContractVersion}, Model Routing Contract v${contract.modelRoutingContractVersion} and exact Safe Core ${contract.safeCoreVersion} accurately; historical labels belong only in changelog/migration history.`,'long-term current identity invariant');
  out=out.replace('shared review/safety/Judgment Lifecycle/Runtime/Provider semantics remain exact-pinned Safe Core Family v4.','shared review/safety/Judgment Lifecycle/Runtime/Provider/Model Routing semantics remain exact-pinned Safe Core Family v4.');
  out=out.replace('README/README.zh-CN/OPERATIONS/SECURITY/ARCHITECTURE and `product-contract.json`','README/README.zh-CN/OPERATIONS/SECURITY/ARCHITECTURE/LONG_TERM_ASSET and `product-contract.json`');
  return out;
}
function syncCompose(text){return text.replace(/(ghcr\.io\/jiying2007\/codex-review-service:)\d+\.\d+\.\d+/g,`$1${contract.serviceVersion}`);}
function main(){const files=[['docs/DEPLOYMENT.md',text=>syncDeployment(text)],['docs/DEPLOYMENT.zh-CN.md',text=>syncDeployment(text,{zh:true})],['OPERATIONS.md',syncOperations],['SECURITY.md',syncSecurity],['LONG_TERM_ASSET.md',syncLongTermAsset],['deploy/docker/compose.yaml',syncCompose]];for(const[file,fn]of files){const target=path.join(root,file);fs.writeFileSync(target,fn(fs.readFileSync(target,'utf8')));}}
if(require.main===module)main();
module.exports={routingFields,syncDeployment,syncOperations,syncSecurity,syncLongTermAsset,syncCompose};
