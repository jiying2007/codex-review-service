...
function readmeBlock(zh=false){const rows=zh?[
  START,'', '`product-contract.json` 是唯一机器校验的当前产品身份：','',
  `- Service：**${contract.serviceVersion}**`,
  `- Database Schema：**${contract.databaseSchemaVersion}**`,
  `- Config Schema：**${contract.configSchemaVersion}**`,
  `- Policy Schema：**${contract.policySchemaVersion}**`,
  `- Review Receipt：**${contract.reviewReceiptVersion}**`,
  `- Safe Contract：**${contract.safeContractVersion}**`,
  `- Safe Core：精确提交 \`${contract.safeCoreCommit}\``,
  `- Quality Platform：**${core.qualityPlatformVersion}**`,
  `- Review Profile：**${core.reviewProfileVersion}**`,
  `- Impact Evidence：**${core.impactEvidenceVersion}**`,
  `- Analyzer Finding：**${core.analyzerFindingVersion}**`,
  `- Native/systemd Node.js：**Node 22 LTS >=${contract.minimumNodeVersion}，或 Node 24 LTS >=${contract.canonicalNodeVersion}**；明确不支持 Node 23`,
  `- 官方 Docker runtime：**Node ${contract.canonicalNodeVersion}**`,
  `- GitLab Self-Managed 兼容下限：**${contract.minimumGitLabVersion}**`,
  '- GitLab 推荐策略：生产环境应运行 **GitLab 官方仍支持的版本**，兼容下限不代表建议长期停留在旧版本','',END
]:[
  START,'','`product-contract.json` is the single machine-checked source for the current product identity:','',
  `- Service: **${contract.serviceVersion}**`,
  `- Database Schema: **${contract.databaseSchemaVersion}**`,
  `- Config Schema: **${contract.configSchemaVersion}**`,
  `- Policy Schema: **${contract.policySchemaVersion}**`,
  `- Review Receipt: **${contract.reviewReceiptVersion}**`,
  `- Safe Contract: **${contract.safeContractVersion}**`,
  `- Safe Core: exact commit \`${contract.safeCoreCommit}\``,
  `- Quality Platform: **${core.qualityPlatformVersion}**`,
  `- Review Profile: **${core.reviewProfileVersion}**`,
  `- Impact Evidence: **${core.impactEvidenceVersion}**`,
  `- Analyzer Finding: **${core.analyzerFindingVersion}**`,
  `- Native/systemd Node.js: **22 LTS >=${contract.minimumNodeVersion}, or 24 LTS >=${contract.canonicalNodeVersion}**; Node 23 is intentionally unsupported`,
  `- Canonical Docker runtime: **Node ${contract.canonicalNodeVersion}**`,
  `- GitLab Self-Managed compatibility floor: **${contract.minimumGitLabVersion}**`,
  '- GitLab recommendation: run a **vendor-supported GitLab release**; the compatibility floor is not a recommendation to stay on an old release','',END
];return rows.join('\n');}
...
function ensureReadmeBlock(text,{zh=false}={}){let out=text;if(out.includes(START)&&out.includes(END))out=replaceGenerated(out,readmeBlock(zh));else{const heading=zh?'## 产品契约':'## Product contract';const boundary=zh?'\nGitLab 兼容通过':'\nGitLab compatibility';const start=out.indexOf(heading),end=out.indexOf(boundary,start);if(start<0||end<start)throw new Error(`Unable to locate ${heading} identity section`);out=out.slice(0,start+heading.length)+'\n\n'+readmeBlock(zh)+'\n'+out.slice(end);}
  out=zh?out.replace(/当前正式产品基线为 \*\*v\d+\.\d+\.\d+\*\*：/,'当前产品身份由下方机器生成契约区块定义：').replace(/codex-review-service-\d+\.\d+\.\d+\.tgz/g,`codex-review-service-${contract.serviceVersion}.tgz`).replace(/Service v\d+\.\d+ 不改变共享 Review 协议/g,`Service v${contract.serviceVersion.split('.').slice(0,2).join('.')} 不改变共享 Review 协议`).replace(/Schema \d+ verification pass/g,`Schema ${contract.databaseSchemaVersion} verification pass`):out.replace(/Service \*\*v\d+\.\d+\.\d+\*\* is the current compatibility\/operations baseline:/,'The current product identity is defined by the machine-generated contract block below:').replace(/codex-review-service-\d+\.\d+\.\d+\.tgz/g,`codex-review-service-${contract.serviceVersion}.tgz`).replace(/Service v\d+\.\d+ does not change the shared review protocol/g,`Service v${contract.serviceVersion.split('.').slice(0,2).join('.')} does not change the shared review protocol`).replace(/Schema \d+ verification pass/g,`Schema ${contract.databaseSchemaVersion} verification pass`);
  return out;}
...
module.exports={START,END,block,readmeBlock,replaceGenerated,ensureReadmeBlock,productContractDoc};
