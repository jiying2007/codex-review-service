...
const {versionAtLeast}=require('../src/gitlab-capabilities');
...
async function main(){const suffix=`codex-review-${Date.now()}`,group=await request('POST','/groups',{body:{name:suffix,path:suffix},expected:[201]});let project;try{project=await request('POST','/projects',{body:{name:'service-e2e',path:'service-e2e',namespace_id:group.id,initialize_with_readme:true,default_branch:'main'},expected:[201]});const pid=project.id;
  ...
  const client=new GitLabClient(config),version=await client.getVersion(),capabilities=await client.getCapabilities();assert.ok(versionAtLeast(version.version,contract.minimumGitLabVersion),`unexpected GitLab version ${version.version}`);
  ...
  process.stdout.write(JSON.stringify({ok:true,gitlabVersion:version.version,profile:capabilities.profile,diffCompleteness:capabilities.diffCompleteness,projectId:pid,mrIid:mr.iid,diffCount:diffs.items.length})+'\n');
