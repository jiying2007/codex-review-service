...
- Review Receipt：**5**
- Safe Contract：**2**
- Safe Core：精确提交 `8375907712db37492aff1ac0d0013e2753b1f6ab`
- Quality Platform：**3**
- Review Profile：**1**
- Profile Pack：**1**
- Impact Evidence：**2**
...
任一 profile 只要无法证明 diff 完整，就会在调用 Codex 前 fail closed。真实 Provider CI 覆盖 GitLab CE **14.6.1、17.11.7、19.3.0**。Safe Core 仍是 Family v4，Service v7.1 不改变共享 Review 协议。
...
优先把经过验证的 `codex-review-service-7.1.0.tgz` 安装到 `/opt/codex-review-service`。
